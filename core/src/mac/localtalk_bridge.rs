...
pub struct LocalTalkBridge {
    ...
    node_address: u8,
    /// Pending CTS response to inject (dest, src)
    pending_cts: Option<(u8, u8)>,
...
impl LocalTalkBridge {
    ...
    pub fn handle_tx_packet(&mut self, llap: &[u8]) {
        ...
        trace!(
            "LocalTalk TX: dest={} src={} type={:02X} len={}",
            dest,
            src,
            ptype,
            llap.len()
        ...
        match ptype {
            0x84 => {
                ...
                if dest != 0xFF {
                    ...
                    self.pending_cts = Some((src, dest));
                    trace!("LocalTalk: RTS -> synthesizing CTS");
                }
            ...
            0x85 => {
                ...
                trace!("LocalTalk: CTS not sent over network");
            }
            _ => {
    ...
    pub fn poll(&mut self) -> bool {
        ...
        loop {
            match self.socket.recv_from(&mut buf) {
                Ok((len, src_addr)) => {
                    ...
                    // Extract LLAP packet
                    let llap = &buf[4..len];
                    self.handle_rx_packet(llap, src_addr);
                    received_any = true;
                }
        ...
        received_any || self.pending_cts.is_some() || !self.rx_queue.is_empty()
    }

    /// Handle a received LLAP packet from the network
    fn handle_rx_packet(&mut self, llap: &[u8], _src_addr: SocketAddr) {
        if llap.len() < 3 {
            return;
        }
        ...
        // Accept: broadcast (0xFF), or our node address, or if we don't have an address yet
    ...
    pub fn read_to_scc(&mut self) -> Option<Vec<u8>> {
        ...
        if let Some((dest, src)) = self.pending_cts.take() {
    ...
    pub fn write_from_scc(&mut self, data: &[u8]) {
        ...
        // Try to extract complete LLAP packets from the buffer
        ...
        while self.tx_buffer.len() >= 3 {
            ...
            let packet_len = if ptype >= 0x80 {
                ...
                if ddp_len == 0 || ddp_len > MAX_LLAP_SIZE - 3 {
                    // Invalid length - might be corrupted, try to skip
                    warn!("LocalTalk: invalid DDP length {}, skipping byte", ddp_len);
