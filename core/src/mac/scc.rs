...
struct SccChannel {
    ...
    /// Pre-buffered RX byte (like Mini vMac's RxBuff)
    lt_rx_buff: u8,
...
impl Scc {
    ...
    /// Uses pre-buffered approach like Mini vMac - RxBuff contains the current byte,
    /// reading advances to the next byte
    fn read_data_sdlc(&mut self, chi: usize) -> u8 {
        ...
        // Advance to next byte (like Mini vMac's SCC_RxBuffAdvance)
        self.lt_rx_buff_advance(chi);

        value
    }

    /// Advance the RX buffer to the next byte (like Mini vMac's SCC_RxBuffAdvance)
    ...
    fn get_irq_pending(&self) -> Option<SccIrqPending> {
        ...
        // Channel A receive (not typically used for LocalTalk but keep for completeness)
    ...
    fn write_ctrl(&mut self, ch: SccCh, val: u8) {
        ...
        match reg {
            ...
            3 => {
                ...
                // If RX is being disabled, clear the frame
                ...
                // Enable SDLC mode if SDLC address search is enabled
                // This is an alternative indicator that the Mac wants SDLC/LocalTalk mode
    ...
    pub fn push_rx_frame(&mut self, ch: SccCh, frame: Vec<u8>) {
        ...
        // Pre-buffer the first byte (like Mini vMac's SCC_RxBuffAdvance call in LocalTalkTick)
        self.lt_rx_buff_advance(chi);

        // Set chr_avail to signal data is ready
